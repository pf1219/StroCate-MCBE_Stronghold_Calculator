...
data=pickle.load(open("stronghold16.p","rb"))
...
for i in range(len(cand)):
    if cand[i][0]%200 in range(50,151):
        if cand[i][1]%200 in range(50,151):
...
dist_list=[]
for i in range(len(cand)):
    if cand[i][0]%27<=17 and cand[i][1]%27<=17:
        dist_list.append(round((cand[i][0]**2+cand[i][1]**2)**0.5))
...
a=open("pre_prob16.csv","w",newline="")
import csv
f=csv.writer(a)
...
plt.plot(range(251),adj_count,color='blue')
plt.xticks([25*i for i in range(11)])
plt.show()
